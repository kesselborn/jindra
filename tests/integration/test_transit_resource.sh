it_should_correctly_transit_files() {
  ...
  ${jindra_cli} -b ${build_no} -c transit-test.yaml all|${kubectl} apply -f-
  ...
  ${kubectl} delete pod jindra.transit-test.${build_no}
}

it_should_not_merge_two_transit_directories() {
  local build_no=${RANDOM}
  ${jindra_cli} -b ${build_no} -c transit-test2.yaml all|${kubectl} apply -f-
  defer "${kubectl} delete pod jindra.transit-test.${build_no}"
