...
it_should_reject_invalid_pipeline() {
  ...
  defer "${kubectl} delete jindrapipeline internal-validation-test"
...
it_should_set_default_values() {
  ...
  defer "${kubectl} delete jindrapipeline internal-modification-test"

  local src=$(${kubectl} get jindrapipeline internal-modifications-test -oyaml)
  assert_match "echo '${src}'" "restartPolicy: Never"
  assert_match "echo '${src}'" 'jindra.io/build-no-offset: "0"'
}

it_should_update_pipeline() {
  local res=$(${kubectl} apply --wait -f minimal-pipeline.yaml 2>&1)
  defer "${kubectl} delete jindrapipeline minimal-pipeline"
